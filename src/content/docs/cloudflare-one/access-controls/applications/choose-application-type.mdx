...
Self-hosted applications can also protect a Cloudflare Worker directly by name, rather than by hostname or IP. When you select a Worker as the destination, you can choose whether Access covers the Worker only in production, or the Worker and all of its preview branches.
